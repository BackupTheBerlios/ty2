...
public class Hero extends Being {
	...
	public static final int HUNGERLEVEL=250000;
	...
	public Quest getQuest(String s) {
		...
		for (int i=0; i<quests.length;i++) {
			if (quests[i].questName().equals(s)) return quests[i]; 
		}
		return null;		
	}
	...
	public Quest[] getQuests() {
		...
		for (int i=0;i<quests.length;i++) {
			...
			if (QuestApp.debug) Game.message(quests[i].getName()+" : "+quests[i].getClass().getName());		 
		}
	...
	public void action(int t) {
		...
		// SPECIAL ABILITIES
		// thief searches
		for (int i=RPG.po(t*getStat(RPG.ST_SEARCHING),2000); i>0; i--) searchAround();
