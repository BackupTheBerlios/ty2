...
class GameScreen extends Screen
{
	...
	public GameScreen()
	{
		...
		// create our world!!
		/*
		 *  Map m=new Town(51,51);
		 *  enterMap(m,40,20);
		 *  Map f=new DeepForest(71,71);
		 *  m.entrance=new Portal(0);
	...
	public void keyAction(KeyEvent e)
	{
		...
		int sx        = h.x;//This is the x position where the hero starts
		int sy        = h.y;//This is the y position where the hero starts
		int shps      = h.hps;//These are the hitpoints when the hero starts
		//If at the end of this loop, the hero has not moved, then he has stopped running
		...
		{
			if("123456789avix>?()-=#:".indexOf(k) < 0)
				return;
		}
		else
		//You shouldnt run in Worldmap
			if("123456789".indexOf(k) >= 0)
			if(e.isShiftDown())
			{
				running = true;
				runKey = e;
		...
		if(k == ':')
		{
			if(!QuestApp.debug)
				return;
			String command  = Game.getLine("[DEBUG MODE] : What is your bidding?");
		...
		if(k == 'c')
		{
			...
			if(map.countNearbyMobiles(h.x, h.y, 1) > 2)
			{
				...
				try
				{
					person = map.getNearbyMobile(h.x, h.y, 1);
				}
		...
		if(k == 'g')
		{
			...
			if(map.countNearbyMobiles(h.x, h.y, 1) > 2)
			{
				...
				try
				{
					mobile = map.getNearbyMobile(h.x, h.y, 1);
				}
				catch(Exception anyex)
				{
				}
			...
			if((mobile != null) && (!h.isHostile(mobile)))
			{
				...
				if(gift != null)
				{
					...
					if((gift.y > 0) && (!h.inv.clearUsage(gift.y)))
						return;
		...
		if(k == '?')
		{
			questapp.switchScreen(new InfoScreen(
				"Key Commands:\n"
...
//       +"  j = jump\n"
		...
		running = (sx != h.x || sy != h.y) && running && (shps >= h.hps);
